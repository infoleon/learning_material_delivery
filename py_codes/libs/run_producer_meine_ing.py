...
def run_producer_ing(sim_fs, site_fs, crop_fs, weather_fs, working_dir, path_env, weather_header_order,
                  fit_parameters, fit = True, workstep = 0, sub_step=0):
    ...
    for i in range(leng):
        ...
        #print(env["cropRotation"][0]["worksteps"])
        
        
        #for key, value in env["cropRotation"][0]["worksteps"][0].items():
        #    print(f"{key}: {value}")
        
        """
        print("-----------")
        print("-----------")
        for i in env["cropRotation"][workstep]["worksteps"]:
            print(i)
            print("------------------")
        
        print("-----------")
        print("-----------")
        print(len(env["cropRotation"][workstep]["worksteps"]))
        
        sys.exit()
        
        for key, value in env["cropRotation"][0]["worksteps"][workstep].items():
            #print(f"{key}: {value}")
            print(f"{key}")
        sys.exit()
        ...
        if fit:
            # Crop changes
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["cultivar"]["StageTemperatureSum"][0] = fit_parameters[0] # Vetor
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["cultivar"]["VernalisationRequirement"][0] = fit_parameters[1]
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["cultivar"]["MaxAssimilationRate"] = fit_parameters[2]
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["species"]["RootPenetrationRate"]  = fit_parameters[3]
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["cultivar"]["DaylengthRequirement"][0] = fit_parameters[4] # Vetor
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["species"]["BaseTemperature"] = fit_parameters[5] # baset
            env["cropRotation"][workstep]["worksteps"][sub_step]["crop"]["cropParams"]["cultivar"]["SpecificLeafArea"][0] = fit_parameters[6]
