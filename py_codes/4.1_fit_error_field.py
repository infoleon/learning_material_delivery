...
### To train
data_true_soc2 =  data_true[ data_true["Horizon_number"] == 2 ]  #["SOC"]
...
coarse_map_f = path_coarse_map + "SOC_coarse_layer2.tif"
soc2_coarse_bands, meta_coarse = read_pic(coarse_map_f)
soc2_coarse = soc2_coarse_bands[0]  # (Hc,Wc)
...
xs = data_true_soc2["Easting"].to_numpy(float)
ys = data_true_soc2["Northing"].to_numpy(float)
r_c, c_c = rowcol(meta_coarse["transform"], xs, ys, op=np.floor)
r_c = r_c.astype(int); c_c = c_c.astype(int)
Hc, Wc = soc2_coarse.shape
m_in = (r_c>=0) & (r_c<Hc) & (c_c>=0) & (c_c<Wc)


# sample features at the GT points (no need to merge on 'idx')
data_true_soc2 = data_true_soc2.copy()
data_true_soc2["SOC_coarse"] = np.nan
data_true_soc2.loc[m_in, "SOC_coarse"] = soc2_coarse[r_c[m_in], c_c[m_in]]
data_true_soc2["Tra_sum_pt"] = np.nan
data_true_soc2.loc[m_in, "Tra_sum_pt"] = tra_mon[r_c[m_in], c_c[m_in]]
data_true_soc2["p95_pt"] = np.nan
data_true_soc2.loc[m_in, "p95_pt"] = p95[r_c[m_in], c_c[m_in]]
data_true_soc2["Yield_pt"] = np.nan
data_true_soc2.loc[m_in, "Yield_pt"] = yield_max[r_c[m_in], c_c[m_in]]


data_true_soc2["SOC_true"] = np.nan
data_true_soc2.loc[m_in, "SOC_true"] = soc_true[r_c[m_in], c_c[m_in]]


train_df = (data_true_soc2.loc[m_in, ["SOC", "SOC_coarse", "Tra_sum_pt", "Yield_pt",  "p95_pt"]]   )
...
Hc, Wc = soc2_coarse.shape
valid = np.isfinite(soc2_coarse) & np.isfinite(tra_mon) & np.isfinite(yield_max) & np.isfinite(p95) & np.isfinite(soc_true)
...
soc_corrected = np.where(valid, soc2_coarse - pred_err, np.nan)
...
df_out = pd.DataFrame({
    ...
    "SOC_coarse_layer2": soc2_coarse[valid],
    "SOC_corrected_layer2": soc_corrected[valid],
    "Tra_sum":  tra_mon[valid],
    "Yield_max": yield_max[valid],
    "p95": p95[valid],
    "SOC_true_layer2": soc_true[valid]
})

out_csv = os.path.join(base_path, "outputs_fine", "soc_layer2_corrected.csv")
